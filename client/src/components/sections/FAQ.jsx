        ...
        <div className="space-y-4">
          {faqs.map((faq, index) => (
            <RevealWrapper key={faq.id} delay={100 * (index + 1)}>
              <div className="bg-white/70 backdrop-blur-sm rounded-xl shadow-lg border border-white/50 p-4 sm:p-6 transition-all duration-300 hover:shadow-xl group">
                <div
                  className="flex justify-between items-center cursor-pointer hover:text-cyan-600 transition-colors duration-300"
                  onClick={() => toggleAccordion(faq.id)}
                >
                  <span className="font-semibold text-sm sm:text-base text-gray-800 group-hover:text-cyan-600 transition-colors duration-300 pr-4">{faq.question}</span>
                  <ChevronRight 
                    className={`w-4 sm:w-5 h-4 sm:h-5 transition-transform duration-300 text-cyan-500 flex-shrink-0 ${
