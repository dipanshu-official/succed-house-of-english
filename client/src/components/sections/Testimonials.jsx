        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {testimonials.map((testimonial, index) => (
            <RevealWrapper key={testimonial.id} delay={100 * (index + 1)}>
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-8 border border-white/50 transition-all duration-500 hover:-translate-y-4 hover:shadow-2xl group relative overflow-hidden">
                {/* Background Gradient */}
                ...
                <div className={`w-20 h-20 rounded-full ${testimonial.avatarBg} mx-auto mb-6 flex items-center justify-center ${testimonial.iconColor} transform group-hover:scale-110 transition-transform duration-300 shadow-lg relative z-10`}>
                  <User className="w-10 h-10" />
                </div>
                <p className="italic text-gray-700 mb-6 leading-relaxed relative z-10 group-hover:text-gray-800 transition-colors duration-300">
                  "{testimonial.message}"
                </p>
                <p className="font-semibold text-gray-900 group-hover:text-rose-600 transition-colors duration-300 relative z-10">
